...
<template title="CardAdSingle">
    ...
    <div class="bg-white p-4 rounded-lg shadow-md">
        ...
        <p class="text-gray-500 mb-2">Valid until: {{ formatDate(date) }}</p>

        <!-- Condição para mostrar botão de edição -->
        <div v-if="canEdit" class="flex items-center">
            <button @click="editAd" class="bg-blue-500 text-white px-4 py-2 rounded-md">Edit Ad</button>
            <button @click="Delete" class="bg-blue-500 text-white px-4 py-2 rounded-md">Delete</button>
        </div>
        <div v-if="canEdit" class="flex items-center">
        </div>
    </div>
